...
export const aiTakesOverStore = reactive({
  games: [],
  
  loadGames() {
    const saved = localStorage.getItem('aiTakesOverGames')
    if (saved) {
  ...
  saveGames() {
    localStorage.setItem('aiTakesOverGames', JSON.stringify(this.games))
  },
