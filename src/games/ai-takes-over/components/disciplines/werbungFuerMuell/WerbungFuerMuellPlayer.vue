<template>
  <div class="werbung-fuer-muell-player">
    <!-- Phase: Writing - Player writes their ad pitch -->
    <div v-if="phase === 'writing'" class="writing-phase">
      ...
      <div class="product-container">
        <p class="product-label">{{ t('werbungFuerMuell.player.productLabel') }}</p>
      ...
      <div class="input-section">
        <label class="input-label">{{ t('werbungFuerMuell.player.yourPitch') }}</label>
        <textarea
          v-model="pitchInput"
          :placeholder="t('werbungFuerMuell.player.placeholder')"
      ...
      >
        <span v-if="hasSubmitted">{{ t('werbungFuerMuell.player.submitted') }}</span>
        <span v-else>{{ t('werbungFuerMuell.player.submitButton') }}</span>
      </button>

      <!-- Hint -->
      <p v-if="charCount < minLength && charCount > 0 && !hasSubmitted" class="hint-text">
        {{ t('werbungFuerMuell.player.needMore', { min: minLength }) }}
      </p>
    </div>
    ...
    <div v-else-if="phase === 'reveal'" class="waiting-phase">
      <div class="waiting-content">
        ...
        <p class="waiting-text">{{ t('werbungFuerMuell.player.watchHost') }}</p>
        <p class="waiting-subtext">{{ t('werbungFuerMuell.player.revealingPitches') }}</p>
      </div>
    </div>
    ...
    <div v-else-if="phase === 'voting'" class="voting-phase">
      <div class="vote-header">
        <p class="vote-instruction">{{ t('werbungFuerMuell.player.voteForBest') }}</p>
        <div class="timer-small">{{ timeRemaining }}s</div>
      </div>

      <!-- Product reminder -->
      <div class="product-reminder">
        <p class="product-mini">{{ currentProduct?.name }}</p>
      </div>
      ...
      <div class="vote-options">
        ...
        >
          ...
          <p class="option-text">{{ truncatePitch(option.pitch) }}</p>
          <span v-if="option.playerId === currentPlayerId" class="own-tag">DEIN PITCH</span>
        </button>
      </div>
      ...
      >
        {{ t('werbungFuerMuell.player.confirmVote') }}
      </button>
      ...
      <div v-if="hasVoted" class="vote-confirmed">
        ...
        <span>{{ t('werbungFuerMuell.player.voteConfirmed') }}</span>
      </div>
    </div>
    ...
    <div v-else-if="phase === 'results'" class="results-phase">
      <div class="results-content">
        ...
        <h3 class="results-title">{{ t('werbungFuerMuell.player.roundComplete') }}</h3>
        <div class="your-score">
          <span class="score-label">{{ t('werbungFuerMuell.player.yourScore') }}</span>
          <span class="score-value">{{ playerScore }}</span>
          <span class="score-unit">{{ t('werbungFuerMuell.player.points') }}</span>
        </div>
        <p class="results-hint">{{ t('werbungFuerMuell.player.watchLeaderboard') }}</p>
      </div>
    </div>
    ...
    <div v-else class="waiting-phase">
      <div class="waiting-content">
        ...
        <p class="waiting-text">{{ t('werbungFuerMuell.player.waiting') }}</p>
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  setup(props, { emit }) {
    ...
    const maxLength = WERBUNG_FUER_MUELL_CONFIG.maxInputLength
    const minLength = WERBUNG_FUER_MUELL_CONFIG.minInputLength

    const timerPercent = computed(() => (props.timeRemaining / props.maxTime) * 100)
    const charCount = computed(() => pitchInput.value.length)
    const canSubmit = computed(() => charCount.value >= minLength && charCount.value <= maxLength)

    const truncatePitch = (pitch) => {
      if (pitch.length > 80) return pitch.substring(0, 80) + '...'
      return pitch
    }

    const submitPitch = () => {
      if (!canSubmit.value || hasSubmitted.value) return
      emit('submit-pitch', pitchInput.value.trim())
